...
public class CrnkUtil {
	 ...
	 * @return Relationship
	 */
	public Relationship getRelationshipObject(String id, String objectType,ObjectNode linkObject) {
		...
		if(linkObject !=null)
			relationshipObject.setLinks(linkObject);
	...
	public ErrorData getErrorBuilderData(String code, String title, String detail, String sourcePointer,
			String sourceParameter) {
		ErrorData erroData = ErrorData.builder().setDetail(detail).setCode(code).setTitle(title).setSourcePointer(sourcePointer).setSourceParameter(sourceParameter).build();
		return erroData;
	}
